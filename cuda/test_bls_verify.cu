// BLS Signature Verification on GPU
// Tests against oracle: e(294*G1, G2) * e(-7*G1, 42*G2) == identity
// This is: e(sig, G2) * e(-H(m), pk) where sk=42, H(m)=7*G1, sig=42*7*G1=294*G1
...
#include "g2_coeffs.h"
// 42*G2 (public key) prepared coefficients
#include "pk42_coeffs.h"

// ==================== All proven field arithmetic (same as bench_full_pairing.cu) ====================
...
// Load coeff from array
__device__ void lc(const uint64_t*co,int idx,Fp2&c0,Fp2&c1,Fp2&c2){int b=idx*36;for(int i=0;i<6;i++){c0.c0.v[i]=co[b+i];c0.c1.v[i]=co[b+6+i];}for(int i=0;i<6;i++){c1.c0.v[i]=co[b+12+i];c1.c1.v[i]=co[b+18+i];}for(int i=0;i<6;i++){c2.c0.v[i]=co[b+24+i];c2.c1.v[i]=co[b+30+i];}}
...
// Multi-miller loop for 2 pairs
__device__ __noinline__ Fp12 multi_miller_2(
    const G1Affine&p1, const uint64_t*q1,
    ...
    for(int b=63;b>=0;b--){
        ...
        Fp2 c0,c1,c2;
        lc(q1,ci,c0,c1,c2); f=ell(f,c0,c1,c2,p1);
        lc(q2,ci,c0,c1,c2); f=ell(f,c0,c1,c2,p2);
        ci++;
        if(bit){
            lc(q1,ci,c0,c1,c2); f=ell(f,c0,c1,c2,p1);
            lc(q2,ci,c0,c1,c2); f=ell(f,c0,c1,c2,p2);
...
__device__ __noinline__ Fp12 final_exp(const Fp12&f_in){
    ...
    t2=fp12_mul(fp12_frobenius(fp12_frobenius(t2)),t1);f=t2;t1=fp12_conj(fp12_sqr(t2));
    Fp12 t3=cyc_exp(t2),t4=fp12_mul(t1,t3);t1=cyc_exp(t4);t4=fp12_conj(t4);f=fp12_mul(f,t4);
    t4=fp12_sqr(t3);t0=cyc_exp(t1);t3=fp12_mul(t3,t0);t3=fp12_frobenius(fp12_frobenius(t3));f=fp12_mul(f,t3);
    t4=fp12_mul(t4,cyc_exp(t0));f=fp12_mul(f,cyc_exp(t4));t4=fp12_mul(t4,fp12_conj(t2));t2=fp12_mul(t2,t1);
    t2=fp12_frobenius(fp12_frobenius(fp12_frobenius(t2)));f=fp12_mul(f,t2);t4=fp12_frobenius(t4);f=fp12_mul(f,t4);return f;}

// ==================== BLS Verify Test ====================
__global__ void test_bls_verify() {
    // sig = 294*G1
    G1Affine sig;
    sig.x.v[0]=0xd27b1adaea06e32cULL;sig.x.v[1]=0x9079033c644ae1d9ULL;
    sig.x.v[2]=0xf154b307a1249c34ULL;sig.x.v[3]=0xa365af8b574fe9d6ULL;
    sig.x.v[4]=0x375b89d156410186ULL;sig.x.v[5]=0x139b61eeb595cf47ULL;
    sig.y.v[0]=0xd973686bc9912933ULL;sig.y.v[1]=0x40d7e6761b92732fULL;
    sig.y.v[2]=0x6b43adf272a19617ULL;sig.y.v[3]=0x31388f4c360d31deULL;
    sig.y.v[4]=0x588138872a0f1626ULL;sig.y.v[5]=0x0a0e79be45d84809ULL;

    // -H(m) = -(7*G1)
    G1Affine neg_hm;
    neg_hm.x.v[0]=0xbf6f80fad9849c75ULL;neg_hm.x.v[1]=0x018298254a48192dULL;
    neg_hm.x.v[2]=0xa8588f9235e2e40dULL;neg_hm.x.v[3]=0x5508d390e218ff49ULL;
    neg_hm.x.v[4]=0xf29c6756cc2dd13aULL;neg_hm.x.v[5]=0x0d3056fc0db4365fULL;
    neg_hm.y.v[0]=0x25301550dae86c14ULL;neg_hm.y.v[1]=0x3140795267108347ULL;
    neg_hm.y.v[2]=0xc5d7e01597b162a4ULL;neg_hm.y.v[3]=0x1c0d85a74c2e54c0ULL;
    neg_hm.y.v[4]=0xf66a4c922bdcc305ULL;neg_hm.y.v[5]=0x10d66042e1a3e5acULL;

    printf("BLS Signature Verification on GPU\n");
    printf("  sk=42, H(m)=7*G1, sig=294*G1, pk=42*G2\n");
    printf("  Check: e(sig, G2) * e(-H(m), pk) == identity\n\n");

    // multi_miller_loop([(sig, G2_gen), (-H(m), 42*G2)])
    Fp12 ml = multi_miller_2(sig, G2_COEFFS, neg_hm, PK42_COEFFS);

    // Compare miller loop with oracle
    printf("Miller loop c0.c0.c0:\n");
    Fp* fps = (Fp*)&ml;
    printf("  GPU=[%016llx,%016llx,%016llx,%016llx,%016llx,%016llx]\n",
    ...
    // Oracle expected:
    printf("  EXP=[b97551d171f25a63,b405e8d5b30b0330,551a5fdbc4b56b5d,53b8014255b94340,797dab7318650bc8,0f7e9a6fa7310cbb]\n");

    bool ml_match = (fps[0].v[0]==0xb97551d171f25a63ULL && fps[0].v[5]==0x0f7e9a6fa7310cbbULL);
    printf("  Miller loop MATCH: %s\n\n", ml_match ? "YES!" : "NO");

    // Final exponentiation
    Fp12 result = final_exp(ml);
    Fp* rp = (Fp*)&result;

    // Check if result is identity (c0.c0.c0 = FP_ONE, everything else zero)
    bool is_one = fp_eq(rp[0], fp_one());
    bool rest_zero = true;
    for(int i=1;i<12;i++) {
        bool fpz = true;
        for(int j=0;j<6;j++) if(rp[i].v[j]!=0) fpz=false;
        if(!fpz) rest_zero=false;
    }

    printf("Final exp result:\n");
    ...
    printf("  c0.c0.c0 == ONE: %s\n", is_one ? "YES" : "NO");
    printf("  rest == zero: %s\n", rest_zero ? "YES" : "NO");
    printf("\n  *** BLS VERIFY: %s ***\n", (is_one && rest_zero) ? "VALID SIGNATURE" : "INVALID");
}

int main() {
    printf("=== GPU BLS Signature Verification ===\n\n");
    test_bls_verify<<<1,1>>>();
